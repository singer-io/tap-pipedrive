...
class StartDateMockedIntegrationTest(PipedriveMockedBaseTest, unittest.TestCase):
    ...
    def test_start_date_used_when_no_bookmark(
        self,
        _mock_rate,
        _mock_validate,
        mock_write_record,
        ...
        response = MockResponse(
            {
                ...
                "data": [
                    ...
                    {"id": 2, "name": "new", "update_time": "2024-01-11T00:00:00Z"},
                ],
        ...
        with patch.object(PipedriveTap, "streams", [stream]), \
            ...
            tap = PipedriveTap(self.base_config(start_date="2024-01-10T00:00:00Z"), {})
            catalog = self.select_all_streams(tap.do_discover())
            tap.do_sync(catalog)

        self.assertEqual(stream.initial_state, "2024-01-10T00:00:00Z")
        self.assertEqual(mock_write_record.call_count, 1)
        self.assertEqual(mock_write_record.call_args.args[1]["id"], 2)
